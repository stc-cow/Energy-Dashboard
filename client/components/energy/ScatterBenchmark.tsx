...
export default function ScatterBenchmark({ data }: { data: BenchmarkPoint[] }) {
  const d = data.map((p) => ({ name: p.siteName, Diesel: p.dieselLiters, Power: p.powerKw, CO2: p.co2Tons }));
  return (
    <div className="rounded-xl border bg-card p-4 shadow-sm">
      <div className="mb-2 text-sm font-semibold">Benchmark: Diesel vs Power (bubble = CO₂)</div>
