...
function buildHierarchy(rows: any[]): HierarchyResponse {
  ...
  const siteMap = new Map<string, { id: string; name: string; cityId: string; lat: number; lng: number }>();

  for (const r of rows) {
    const regionName = String(r["regionName"] ?? r["Region"] ?? r["region"] ?? "").trim();
    const cityName = String(r["cityName"] ?? r["City"] ?? r["city"] ?? "").trim();
    ...
    if (siteName && !siteMap.has(siteId)) {
      ...
      const lng = toNumber(r["lng"] ?? r["longitude"] ?? r["Lon"] ?? r["long"] ?? 0);
...
function rowsInScope(rows: any[], scope: HierarchyFilter) {
  return rows.filter((r) => {
    const regionId = slug(String(r["regionName"] ?? r["Region"] ?? r["region"] ?? ""));
...
export async function fetchKPIs(scope: HierarchyFilter): Promise<KPIsResponse> {
  ...
  try {
    ...
    const fuelLevels = rows.map((r) => toNumber(r["fuelTankLevelPct"]))
      .filter((n) => n > 0 || n === 0);
    const avgFuel = fuelLevels.length
      ? fuelLevels.reduce((a, b) => a + b, 0) / fuelLevels.length
      : 0;

    const genLoads = rows.map((r) => toNumber(r["generatorLoadFactorPct"]))
      .filter((n) => n > 0 || n === 0);
    ...
    const lowFuelWarnings = Array.from(siteAgg.values())
      .map((x) => ({
        ...
        siteName: x.siteName,
        fuelTankLevelPct:
          x.fuel.length ? Math.round((x.fuel.reduce((a, b) => a + b, 0) / x.fuel.length) * 10) / 10 : 0,
      }))
  ...
  try {
    ...
    if (by === "region") {
      ...
      return {
        scope,
        by,
        data: Array.from(map.entries()).map(([key, v]) => ({ key, name: v.name, dieselLiters: Math.round(v.diesel), energyKwh: Math.round(v.energy) })),
      };
    }
    ...
    return {
      scope,
      by,
      data: Array.from(siteMap.entries()).map(([key, v]) => ({ key, name: v.name, dieselLiters: Math.round(v.diesel), energyKwh: Math.round(v.energy) })),
    };
  } catch {
