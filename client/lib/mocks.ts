import { AlertsResponse, BenchmarkResponse, BreakdownResponse, City, HierarchyFilter, HierarchyResponse, KPIsResponse, Region, Site, TimeSeriesResponse } from "@shared/api";
...
export function mockTimeSeries(scope: HierarchyFilter, granularity: "daily" | "monthly" | "yearly"): TimeSeriesResponse {
  ...
  const points = Array.from({ length: granularity === "daily" ? 30 : granularity === "monthly" ? 12 : 5 }).map((_, idx) => {
    ...
    const base = 1000;
    const diesel = base + Math.round(Math.sin(idx / 3) * 200 + Math.random() * 150);
...
export function mockBreakdown(scope: HierarchyFilter, by: "region" | "site"): BreakdownResponse {
  if (by === "region") {
    const data = regions.map((r) => {
      ...
      const diesel = regionSites.reduce((acc) => acc + 700 + Math.floor(Math.random() * 1600), 0);
