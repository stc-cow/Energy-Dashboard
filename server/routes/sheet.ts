...
function getSheetEndpoint(u: string): SheetEndpoint {
  ...
  if (m) {
    ...
    try { gid = new URL(u).searchParams.get("gid"); } catch {}
...
export const proxySheet: RequestHandler = async (req, res) => {
  ...
  try {
    const ep = getSheetEndpoint(sheetParam) || { kind: "gviz", url: sheetParam };
    const r = await fetch(ep.url);
    if (!r.ok) return res.status(502).json({ error: "fetch failed", status: r.status });
    const ct = r.headers.get("content-type") || "";
    const text = await r.text();
    let rows: any[] = [];
    if (ep.kind === "gviz" || ct.includes("json") || text.trim().startsWith("{") ) {
